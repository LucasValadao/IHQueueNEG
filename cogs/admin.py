...
async def leaderboard_persistent(bot, channel, game):
    ...
    embed = Embed(title=f"🏆 Ranking", color=Color.yellow())
    ...
    async def add_field(data) -> None:
        ...
        embed.add_field(
            name=name,
            value=f"{most_played_role} `{member_name}   MMR: {display_mmr} Wins: {data[2]} Loses: {data[3]} WR: {round(data[5]*100)}%`",
            inline=False,
        )
    ...
    async def setup(self, ctx, game=Param(choices={"League Of Legends": "lol", "Valorant": "valorant", "Overwatch": "overwatch", "Other": "other"})):
        ...
        async def process_setup(region):
            ...
            if game == "lol":
                ...
                display_game = "Other"
            category = await ctx.guild.create_category(name=f"Draft da EG - {display_game}", overwrites=mutual_overwrites)
            queue = await category.create_text_channel(name="Fila")
            match_history = await category.create_text_channel(name="Historico de partidas")
            top_ten = await category.create_text_channel(name="Ranking")
            await self.bot.execute(
            ...
            embed = Embed(
                description="O historico das partidas sera postado aqui!",
                color=Color.red()
            )
            ...
            category = await ctx.guild.create_category(name=f"Jogos de {game} em andamento", overwrites=overwrites)
            ...
            embed = embed = Embed(title="Fila do Draft", description=f"All ongoing {display_game} games will be under this category. Feel free to move it around or change the name.", color=Color.red())
