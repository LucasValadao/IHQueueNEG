...
class Leaderboard(Cog):
    ...
    async def leaderboard(self, ctx, game, type="mmr"):
        ...
        embed = Embed(title=f"🏆 Ranking", color=Color.blurple())
        if ctx.guild.icon:
        ...
        async def add_field(data, current_embed) -> None:
            ...
            if type == 'mvp':
                ...
                embeds[current_embed].add_field(
                    name=name,
                    value=f"{most_played_role} `{member_name}  MMR: {display_mmr} Vitórias: {wins} Derrotas: {losses} WR: {percentage}%`",
                    inline=False,
                )
            else:
                embeds[current_embed].add_field(
                    name=name,
                    value=f"{most_played_role} `{member_name}   Vitórias: {wins} Derrotas: {losses} WR: {percentage}%`",
                    inline=False,
                )
