...
async def start_queue(bot, channel, game, author=None, existing_msg = None, game_id = None):
    ...
    if not st_pref:
        if existing_msg:
            ...
            slot1 = "Sem jogadores ainda..."
            slot2 = "Sem jogadores ainda..."
        embed.add_field(name="Time 1", value=slot1)
        embed.add_field(name="Time 2", value=slot2)
        sbmm = True
    else:
        if existing_msg:
            game_members = await bot.fetch(f"SELECT * FROM game_member_data WHERE game_id = '{game_id}'")
            ...
            blue_value = "Sem jogadores ainda..."
            red_value = "Sem jogadores ainda..."
...
class RoleButtons(ui.Button):
    ...
    async def add_participant(self, inter, button, view) -> None:
        ...
        await self.bot.execute(
            "INSERT INTO game_member_data(author_id, role, team, game_id, queue_id, channel_id, author_name) VALUES($1, $2, $3, $4, $5, $6, $7)",
            inter.author.id,
            label,
            team,
            view.game_id,
...
class LeaveButton(ui.Button):
    def __init__(self, bot, game):
        self.bot = bot
        super().__init__(
            label="Sair da Fila", style=ButtonStyle.red, custom_id=f"{game}-queue:leave"
        )
    ...
    async def callback(self, inter):
        ...
        if await view.has_participated(inter, view.game_id):
            ...
            for button in view.children:
                if button.label in ["Sair da Fila", "Switch Team", "Duo"]:
                    continue
...
class SwitchTeamButton(ui.Button):
    def __init__(self, bot, game):
        self.bot = bot
        super().__init__(
            label="Trocar de Time", style=ButtonStyle.blurple, custom_id=f"{game}-queue:switch"
        )
...
class ReadyButton(ui.Button):
    def __init__(self, bot, game, game_id, msg = None):
        ...
        super().__init__(
            label="CLIQUE AQUI PARA ACEITAR A FILA!", style=ButtonStyle.green, custom_id=f"{game}-queue:readyup"
        )
    ...
    async def anonymous_team_embed(self, ready_ups):
        ...
        embed.clear_fields()
        embed.description = "ESTES NÃO SÃO OS TIMES FINAIS, AINDA SERÃO RANDOMIZADOS."
    ...
    async def disable_button(self):
        ...
        if self.msg:
            ...
            if not self.msg.components[0].children[0].label == "CLIQUE AQUI PARA ACEITAR A FILA!":
                self.disable_button.stop()
                return
...
class Queue(ui.View):
    ...
    async def check_end(self, inter) -> None:
        ...
        for button in self.children:
            if button.label in ["Leave Queue", "Trocar de Time", "Duo"]:
                continue
